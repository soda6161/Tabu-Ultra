const ENGINE = {
    roomData: null,
    words: [/* 240 KELİMEN BURADA OLMALI */],
    ...
    renderGame(data) {
        ...
        const turnTeam = data.currentTurn; // 'red' veya 'blue'
        
        // Sıra kimdeyse yukarıdaki barı o renge boya
        const indicator = document.getElementById('turn-indicator');
        indicator.className = turnTeam === 'red' ? 'turn-red' : 'turn-blue';
        indicator.textContent = turnTeam === 'red' ? "KIRMIZI TAKIMIN SIRASI" : "MAVİ TAKIMIN SIRASI";

        // Arka planı senin görsellerine göre ayarla
        const roleClass = me.role === 'anlatici' ? 'narrator' : 'listener';
        screen.className = `screen active ${roleClass}-${me.team}`;
        ...
        // MANTIK:
        // 1. Sırası olan ANLATICI: Kelimeleri görür, butonları kullanır.
        // 2. Sırası olmayan ANLATICI: Diğer takımın kelimelerini görür (Discord'dan hileyi denetlemek için).
        // 3. Sırası olan DİNLEYİCİ: Kelimeleri GÖRMEZ (??? yazar), yazı yazma alanı (tahmin) açılır.
        // 4. Sırası olmayan DİNLEYİCİ: Kelimeleri GÖRÜR (hile yapmasınlar diye), ama yazı yazamaz.

        if (me.role === 'anlatici') {
            wordMain.textContent = data.currentWord.m;
            wordForbidden.innerHTML = data.currentWord.f.join('<br>');
            narratorControls.style.display = (me.team === turnTeam) ? 'flex' : 'none';
            guessArea.style.display = 'none';
        } else {
            // Dinleyiciysen
            if (me.team === turnTeam) {
                // SIRA SENDE: Kelimeleri görme, tahmin et
                wordMain.textContent = "???";
                wordForbidden.textContent = "TAKIM ARKADAŞINI DİNLE!";
                guessArea.style.display = 'flex';
            } else {
                // SIRA DİĞER TAKIMDA: Hile denetimi için kelimeleri gör
                wordMain.textContent = data.currentWord.m;
                wordForbidden.innerHTML = data.currentWord.f.join('<br>');
                guessArea.style.display = 'none';
            }
    ...
    startGame() {
        ...
        if (p.length < 4) return alert("En az 4 kişi lazım!");

        NET.roomRef.update({
            status: 'playing',
            currentTurn: 'red', // Oyun kırmızıyla başlar
